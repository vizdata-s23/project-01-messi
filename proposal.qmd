...
The first plot will examine goal scoring trends across world cups in order to examine the effect of new rules on the outcomes of games. We are **planning** to create a scatterplot showing the number of goals scored during each World Cup using `year` and `goals_scored`. We will add annotations on top of this graph to show when certain changes were implemented in the World Cup, hopefully to highlight/explain changes in patterns.

Our second plot will be a deeper analysis in to the effect of penalty kicks. A very controversial and game-deciding call on the field, the World Cup has seen both the introduction and change in frequency (partially due to VAR) of PKs in its history. Our **plan** for the second plot will be a bar graph showing how the frequency of penalty kicks have changed over time. A possible extension on this idea would be including the success ratios of penalty kicks taken and/or whether the penalty was taken during the game or to break a tie at the end.
...
Our first plot we **plan** to make a graph which records the proportion of teams from Europe/South American and other (non European/South American countries) over each year of the world cup. We will create a variable `continent` based off of the countries of the teams. Then, we will calculate the proportion of each continent group for each year of the world cup and plot a time series graph where the x-axis will be year, the y-axis will be the proportion, and color by continent group.

For our second plot, we will explore diversity representation across rounds of a world cup and analyze whether a potential increase in representation in early rounds leads to more representation in later rounds (with later rounds being the 'better' teams as it is an elimination tournament). We **plan** to make a segmented relative frequency bar chart with the rounds ordered on the x axis from beginning to later rounds and fill by continent. Since there are many world cups in our dataset, we will select the most recent world cups for this plot and facet by those world cups.
