...
world_cup_total <- world_cup_total %>%
  mutate(win_conditions = 
           case_when(is.na(win_conditions) & outcome != "D" ~ 
                       paste(world_cup_total$winning_team, "won in regulation"),
                    is.na(win_conditions) & outcome == "D" ~ "Tie",
                                    TRUE ~ win_conditions)) %>%
  mutate(winning_team = if_else(outcome == "D", "Tie", winning_team)) %>%
  mutate(losing_team = if_else(outcome == "D", "Tie", losing_team))
=======
world_cup_total$win_conditions <-
  ifelse(
    is.na(world_cup_total$win_conditions) &
...
>>>>>>> 8317503554b881b6c33442c8241ea253f0869290
```
...
The first plot will examine goal scoring trends across world cups in order to examine the effect of new rules on the outcomes of games as well as changes in play over time. We are **planning** to create a scatterplot showing the number of goals scored per game of each world cup (This is better than just goals as the number of games increases over time as the WC adds more teams). So, our x-axis will be `year` and the y-axis will be the average `goals_scored` per game of that world cup. We will also add annotations on top of this graph to show when certain changes were implemented in the World Cup, hopefully to highlight/explain changes in patterns. We may add another variable such as continent (color) in order to see if the if the power rankings across the world have shifted over time (in other words, have the goals per game of each continent become more balanced over time or grown further apart?). Additionally using continent in the graph will lead us into our second big question.
